...
public class AddLocationToDepartmentTests : DirectoryServiceBaseTests
{
    ...
    [Fact]
    public async Task AddLocation_should_fail_when_location_already_added_to_department()
    {
        // arrange
        AppDbContext dbContext = null;
        ...
        Assert.NotNull(departmentResult);
        Assert.Single(departmentResult.DepartmentLocations);
        Assert.Equal(locationId, departmentResult.DepartmentLocations.Single().LocationId);
    }
